...
public class TreeMapActivity extends BaseLogCatActivity {
    ...
    void onClick(View v) {
        int size = mTreeMap.size();
        switch (v.getId()) {

            case R.id.card_features_tree_map:
                if(null == mFeaturesDialog){
                    String title = "TreeMap特性";
                    String message = getString(R.string.features_tree_map);
            ...
            case R.id.card_add_tree_map:
                UUID uuid = UUID.randomUUID();
                String addKey = generateKey();
                mTreeMap.put(addKey, uuid.toString());
                mKeys.add(addKey);
                addLog("增加元素, key: " + addKey + ", value: " + uuid.toString());
                break;

            case R.id.card_remove_tree_map:
                if (size <= 0) {
                    addLog("TreeMap没有元素!");
                    return;
                }
                ...
                String removeValue = mTreeMap.get(removeKey);
                mTreeMap.remove(removeKey);
                addLog("删除元素, key: " + removeKey + ", value: " + removeValue);
                break;

            case R.id.card_set_tree_map:
                if (size <= 0) {
                    addLog("TreeMap没有元素!");
                    return;
                }
                ...
                String oldValue = mTreeMap.put(putKey, putUuid.toString());
                addLog("TreeMap更改， key：" + putKey + ", 新value：" + putUuid.toString() + "旧value: " + oldValue);
                break;

            case R.id.card_get_tree_map:
                if (size <= 0) {
                    addLog("TreeMap没有元素!");
                    return;
                }
                ...
                String getValue = mTreeMap.get(getKey);
                addLog("TreeMap获取，key：" + getKey + ", value：" + getValue);
                break;

            case R.id.card_go_through_by_iterator_tree_map:
                if (size <= 0) {
                    addLog("TreeMap没有元素!");
                    return;
                }
                ...
                while (hasNext) {
                    ...
                    String valueByIterator = mTreeMap.get(keyByIterator);
            ...
            case R.id.card_go_through_by_list_iterator_tree_map:
                addLog("TreeMap不支持ListIterator");
                break;

            case R.id.card_go_through_tree_map:
                addLog("Set不支持按index遍历");
                break;

            case R.id.card_go_through_by_entry_set_tree_map:
                if (size <= 0) {
                    addLog("TreeMap没有元素!");
                    return;
                }

                Set<Map.Entry<String, String>> entries = mTreeMap.entrySet();
            ...
            case R.id.card_clear_tree_map:
                mTreeMap.clear();
                mKeys.clear();
                addLog("清除完毕");
                break;

            case R.id.card_to_string_tree_map:
                printToString();
                break;

            case R.id.card_sort_tree_map:
                addLog("TreeMap自动支持KeySet排序, 要求Key元素必须实现Comparable接口");
                break;

            case R.id.card_multi_thread_tree_map:
                addLog("TreeMap不同步，不支持多线程读写操作!");
                break;
        }
    }
    ...
    private void printToString(){
        addLog("当前TreeMap: " + mTreeMap.toString());
    }
}
